...
| [21. Multi-Agent Orchestration & Sub-Agent Execution Architecture](./21-multi-agent-architecture-and-sub-agent-execution.md) | Agent-as-a-Tool (Supervisor-Worker) paradigm, sub-agent lifecycle, context isolation, observation bloat suppression, scoped MCP toolkits, SSE telemetry streaming, `AGENTS.md` mandatory delegation rules, 20-iteration budgets, and Web GUI rule management. |
